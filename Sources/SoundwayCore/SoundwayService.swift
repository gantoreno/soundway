...
public enum SoundwayServiceControl {
  ...
  public static func resolveExecutableURL(
    commandName: String, environment: [String: String] = ProcessInfo.processInfo.environment
  ) -> URL? {
    let fileManager = FileManager.default

    if commandName.contains("/") {
      return fileManager.isExecutableFile(atPath: commandName)
        ? URL(fileURLWithPath: commandName) : nil
    }
  ...
  fileprivate static func send(action: SoundwayServiceAction) throws -> SoundwayServiceResponse {
    ...
    socketPath.withCString { pathPointer in
      withUnsafeMutableBytes(of: &address.sun_path) { bytes in
        ...
        strncpy(
          bytes.baseAddress!.assumingMemoryBound(to: CChar.self), pathPointer, bytes.count - 1)
      }
    }
    ...
    guard let response = try? JSONDecoder().decode(SoundwayServiceResponse.self, from: responseData)
    else {
...
internal struct SoundwayDaemonRequestHandler {
  ...
  func handle(_ request: SoundwayServiceRequest) -> SoundwayDaemonRequestResult {
    ...
    case .status:
      return SoundwayDaemonRequestResult(
        response: .success(status: statusProvider()), shouldStop: false)
    case .stop:
...
public final class SoundwayDaemon {
  ...
  public convenience init(configuration: BridgeConfiguration? = nil) throws {
    try self.init(
      configuration: configuration, configurationStore: SoundwayConfigurationStore(),
      deviceDiscovery: AudioDeviceDiscovery())
  }

  internal init<
    ConfigurationStore: SoundwayConfigurationLoading, DeviceDiscovery: SoundwayDeviceDiscovering
  >(
    ...
    self.inputChannelCount = try deviceDiscovery.channelCount(
      for: endpoints.input.id, scope: kAudioObjectPropertyScopeInput)
    self.outputChannelCount = try deviceDiscovery.channelCount(
      for: endpoints.output.id, scope: kAudioObjectPropertyScopeOutput)
  ...
  public func run() throws {
    ...
    let requestHandler = SoundwayDaemonRequestHandler(statusProvider: {
      [engine, config, inputChannelCount, outputChannelCount] in
    ...
    while true {
      ...
      guard let request = try? JSONDecoder().decode(SoundwayServiceRequest.self, from: requestData)
      else {
