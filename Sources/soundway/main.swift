    ...
    case .status:
      do {
        ...
        if let status = response.status {
          ...
          print(
            "channel routing: \(SoundwayStatusFormatting.describeRouting(status.outputChannelMap))")
        ...
        do {
          ...
          let inputChannels = try discovery.channelCount(
            for: endpoints.input.id, scope: kAudioObjectPropertyScopeInput)
          let outputChannels = try discovery.channelCount(
          ...
          print(
            "channel routing: \(SoundwayStatusFormatting.describeRouting(config.outputChannelMap))")
    ...
    case .start:
      do {
        ...
        guard let executableName = CommandLine.arguments.first,
          let executableURL = serviceControl.resolveExecutableURL(
            commandName: executableName, environment: ProcessInfo.processInfo.environment)
        else {
