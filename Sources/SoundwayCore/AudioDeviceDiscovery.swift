...
public struct AudioDeviceDiscovery: Sendable, SoundwayDeviceDiscovering {
    ...
    public func resolveEndpoints(for configuration: BridgeConfiguration) throws -> ResolvedBridgeEndpoints {
    ...
    private static func deviceInfo(for deviceID: AudioDeviceID) throws -> AudioDeviceInfo {
        ...
        let manufacturer = try? stringProperty(selector: kAudioObjectPropertyManufacturer, deviceID: deviceID)
        return AudioDeviceInfo(id: deviceID, name: name, manufacturer: manufacturer)
    }

    private static func stringProperty(selector: AudioObjectPropertySelector, deviceID: AudioDeviceID) throws -> String {
        var address = AudioObjectPropertyAddress(
            mSelector: selector,
            mScope: kAudioObjectPropertyScopeGlobal,
            mElement: kAudioObjectPropertyElementMain
