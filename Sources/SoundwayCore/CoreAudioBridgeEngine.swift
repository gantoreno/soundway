...
public final class CoreAudioBridgeEngine {
  ...
  public init(endpoints: ResolvedBridgeEndpoints, settings: Settings) {
    ...
    self.processor = SoundwayBridgeProcessor(
      settings: .init(
    ...
    self.inputBufferList = AudioBufferList.allocate(
      maximumBuffers: max(1, Int(settings.inputChannelCount)))
  ...
  public func start() throws {
    ...
    do {
      ...
      var inputCallback = AURenderCallbackStruct(
        inputProc: Self.inputDeviceCallback, inputProcRefCon: inputContext)
      var outputCallback = AURenderCallbackStruct(
        inputProc: Self.outputDeviceCallback, inputProcRefCon: outputContext)
  ...
  private static let inputDeviceCallback: AURenderCallback = {
    refCon, _, timeStamp, busNumber, frameCount, _ in
  ...
  private static let outputDeviceCallback: AURenderCallback = {
